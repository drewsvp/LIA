-- Repair migration: re-create item_request_expired_on(),
-- item_request_current_la_date(), and the item_pledges_reject_expired_request
-- trigger that depend on it. Functions and triggers created in migration 0036
-- can be absent from the production database after a clean publish while table
-- columns survive. Using CREATE OR REPLACE and DROP … IF EXISTS makes this
-- migration safe to re-run on databases that already have the objects.
