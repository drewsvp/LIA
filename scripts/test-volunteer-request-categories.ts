...
async function main(): Promise<void> {
  ...
  const categoryPolicy = await pool.query<{ withCheck: string | null }>(
    `select with_check as "withCheck"
       from pg_policies
      where schemaname = 'public'
        and tablename = 'volunteer_request_categories'
        and policyname = 'volunteer_request_categories_member_all'`,
  );
  assert(
    categoryPolicy.rows[0]?.withCheck?.includes("vc.is_active") === true,
    "the applied member relationship policy rejects a newly assigned inactive category",
  );
  ...
  const archived = await request(staff, `/api/admin/requests/volunteer/${validId}/archive`, {});
  await pool.query(`update volunteer_categories set is_active = false where id = $1`, [zuluId]);
  const inactiveReinstate = await request(staff, `/api/admin/requests/volunteer/${validId}/reinstate`, {});
  const inactiveReinstateBody = (await inactiveReinstate.json()) as { message?: string };
  const afterInactiveReinstate = await pool.query<{ status: string }>(
    `select status from volunteer_requests where id = $1`,
    [validId],
  );
  assert(
    archived.ok &&
      inactiveReinstate.status === 409 &&
      inactiveReinstateBody.message?.includes("active volunteer category") === true &&
      afterInactiveReinstate.rows[0]?.status === "archived",
    "an archived request with only an inactive category cannot be reinstated",
    `archive ${archived.status}, reinstate ${inactiveReinstate.status}`,
  );

  await pool.query(`delete from volunteer_request_categories where volunteer_request_id = $1`, [validId]);
  const uncategorizedReinstate = await request(
    staff,
    `/api/admin/requests/volunteer/${validId}/reinstate`,
    {},
  );
  const afterUncategorizedReinstate = await pool.query<{ status: string }>(
    `select status from volunteer_requests where id = $1`,
    [validId],
  );
  assert(
    uncategorizedReinstate.status === 409 &&
      afterUncategorizedReinstate.rows[0]?.status === "archived",
    "an archived uncategorized request cannot be reinstated",
    `status ${uncategorizedReinstate.status}`,
  );

  await pool.query(`update volunteer_categories set is_active = true where id = $1`, [zuluId]);
  await pool.query(
    `insert into volunteer_request_categories (volunteer_request_id, category_id)
     values ($1, $2)`,
    [validId, zuluId],
  );
  const reinstated = await request(staff, `/api/admin/requests/volunteer/${validId}/reinstate`, {});
  assert(reinstated.ok, "an active category allows an archived request to be reinstated");

  const unapproved = await request(staff, `/api/admin/requests/volunteer/${validId}/unapprove`, {});
  const returned = await request(staff, `/api/admin/requests/volunteer/${validId}/return-to-draft`, {
    note: "zz_fixture category correction",
  });
  await pool.query(`update volunteer_categories set is_active = false where id = $1`, [zuluId]);
  const blockedMoveToPending = await request(
    staff,
    `/api/admin/requests/volunteer/${validId}/move-to-pending`,
    {},
  );
  const blockedMoveBody = (await blockedMoveToPending.json()) as { message?: string };
  const afterBlockedMove = await pool.query<{ status: string }>(
    `select status from volunteer_requests where id = $1`,
    [validId],
  );
  assert(
    unapproved.ok &&
      returned.ok &&
      blockedMoveToPending.status === 409 &&
      blockedMoveBody.message?.includes("active volunteer category") === true &&
      afterBlockedMove.rows[0]?.status === "draft",
    "a returned draft cannot move back to review after its only category is deactivated",
    `unapprove ${unapproved.status}, return ${returned.status}, move ${blockedMoveToPending.status}`,
  );
  await pool.query(`update volunteer_categories set is_active = true where id = $1`, [zuluId]);
  const movedToPending = await request(
    staff,
    `/api/admin/requests/volunteer/${validId}/move-to-pending`,
    {},
  );
  assert(movedToPending.ok, "reactivating the assigned category allows the returned draft back into review");
