...
async function main(): Promise<void> {
  ...
  try {
    ...
    // passes and staff schedule mutations. Hold the first pass after it has
    // read the schedule but before it claims an occurrence, then prove both a
    // second scheduler and a pause/cancel save must retry instead of claiming
    // or falsely reporting that stale settings were canceled.
    ...
    let weeklyLookupCount = 0;
    const blockingDeps = {
      ...scheduledDeps,
      getSchedule: async () => concurrentSchedule,
      weeklyOccurrenceAt: async (date: string, minutes: number) => {
        if (weeklyLookupCount === 0) {
          weeklyLookupCount += 1;
          enteredFirst();
          await firstReleased;
        }
        return dal.emailSchedules.pacificWeeklyLocalToInstant(SYSTEM, date, minutes);
      },
    };
    ...
    let staffSaveRan = false;
    const contendingStaffSave = await dal.digestRuns.tryWithSchedulerLock(async () => {
      staffSaveRan = true;
      fakeSchedule = { ...fakeSchedule, active: false, oneTimeAt: null };
    });
    ...
    check(
      "staff pause/cancel cannot report success after a scheduler read stale settings",
      !contendingStaffSave.acquired && !staffSaveRan,
    );
    check(
