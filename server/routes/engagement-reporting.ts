...
export function registerEngagementReportingRoutes(app: Express): void {
  ...
  app.get("/api/dashboard/engagement/export", requireOrganization, async (req: Request, res: Response, next) => {
    try {
      const { orgId } = orgContext(req);
      const parsed = parseFilters(req, orgId);
      if (!parsed.ok) {
      ...
      const lines = [
        ...
        ...report.performance.map((r) =>
          [
            ...
            r.status,
            String(r.cardClicks),
            String(r.detailViews),
            String(r.productLinkClicks),
            String(r.formStarts),
            String(r.selections),
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="engagement-report-${parsed.filters.from}-to-${parsed.filters.to}.csv"`,
      );
      res.send(`${lines.join("\n")}\n`);
    } catch (err) {
      next(err);
