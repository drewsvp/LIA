...
let _urlHeaderCache: { url: string; base64: string; contentType: string } | null = null;

/** Map a MIME type to a matching attachment filename. */
function attachmentNameFor(contentType: string): string {
  if (contentType === "image/jpeg") return "email-header.jpg";
  if (contentType === "image/webp") return "email-header.webp";
  if (contentType === "image/gif") return "email-header.gif";
  return "email-header.png";
}
 ...
 * is set, resolves and encodes it; falls back to the disk PNG when no URL is
 * configured or the fetch fails.
 *
 * Two resolution paths:
 *  - `/storage/images/…` — trusted app-managed upload; read directly from the
 *    object storage adapter (no HTTPS fetch, no SSRF risk, respects the same
 *    2 MB cap).
 *  - Absolute HTTPS URL — external host; passes through the SSRF-safe fetch
...
export async function getEmailHeaderAttachment(): Promise<EmailInlineAttachment> {
  ...
  if (_urlHeaderCache?.url === brandUrl) {
    const ct = _urlHeaderCache.contentType;
    return { Name: attachmentNameFor(ct), Content: _urlHeaderCache.base64, ContentType: ct, ContentID: EMAIL_HEADER_CID_URL };
  }
  try {
    let base64: string;
    let contentType: string;
    if (brandUrl.startsWith("/storage/images/")) {
      // App-managed upload — read from the storage adapter directly.
      // No external HTTP fetch, no SSRF exposure; the URL is generated by
      // storeImage and validated by objectNameFromUrl on every read.
      const { readImage } = await import("../storage/object-storage");
      const img = await readImage(brandUrl);
      if (img.data.length > MAX_HEADER_IMAGE_BYTES) {
        throw new Error(`Brand header image is too large to attach (${img.data.length} bytes; max ${MAX_HEADER_IMAGE_BYTES}).`);
      }
      base64 = img.data.toString("base64");
      contentType = img.contentType;
    } else {
      // External HTTPS URL — run through the full SSRF-safe fetch path.
      // Content-type is not returned by fetchSafeHeaderImage; default to PNG
      // (pre-existing behaviour for external images, not flagged as a gap).
      base64 = await fetchSafeHeaderImage(brandUrl);
      contentType = "image/png";
    }
    _urlHeaderCache = { url: brandUrl, base64, contentType };
    return { Name: attachmentNameFor(contentType), Content: base64, ContentType: contentType, ContentID: EMAIL_HEADER_CID_URL };
  } catch (err) {
